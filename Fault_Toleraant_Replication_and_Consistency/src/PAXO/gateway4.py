...
acceptors = [1,2,3,4,5]
proposals = {}
promise = 0 ## change promise value to check for denial
in_mem_data = {}
...
def RPCServer():
	...
	def acceptRequest(N, val):## leader sends the consensus and the gateway accpets it and make changes to its database by logging the sensor/device events
		...
		if N >= promise:
			...
			print "Accepted write request:",N," Consensus on node:state :",val," my id: 4"
			write_log(N,val)
			return "accepted"
		else:
			return "denied"
	...
	def write_log(N,val): ## to write to its database
		global in_mem_data
		with open('event_log4.txt', 'a+') as fp:
...
def RPCClient():
	node = xmlrpclib.ServerProxy('http://localhost:8004')
