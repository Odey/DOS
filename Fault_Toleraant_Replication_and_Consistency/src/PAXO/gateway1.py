...
def RPCServer():
	...
	def acceptRequest(N, val):	## leader sends the consensus and the gateway accpets it and make changes to its database by logging the sensor/device events
		...
		if N >= promise:
			...
			print "Accepted write request:",N," Consensus on node:state :",val," my id: 1"
			write_log(N,val)
			return "accepted"
		else:
			return "denied"
	...
	def write_log(N,val): ## to write to its database
		global in_mem_data
		with open('event_log1.txt', 'a+') as fp:
...
def RPCClient():
	node = xmlrpclib.ServerProxy('http://localhost:8001')
