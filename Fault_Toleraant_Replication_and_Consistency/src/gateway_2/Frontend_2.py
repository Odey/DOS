...
def ServerFrontend():
	...
	def register(nodeType,name):
		global node_dict, nodeID_dict, no_nodes, flag
		no_nodes=no_nodes+1
		node_dict[name]=nodeType
		nodeID_dict[name]=no_nodes
		print "Registering ",name," with gateway 2 with node ID ", no_nodes
		...
		if(no_nodes == 6):
			...
			print '\n'
			print '='*120
			print "All device registered to gateway - 2 are!!! \n"
	...
	def report_state(timestamp, ID, state, clock,cached,proc=['x']):
		...
		if GC<clock[0]:
			...
			GC=GC+1
		global reportQueue, userEvents, printFlag, cacheHit, cacheMiss
		eventnotify.wait()
		eventnotify.clear()
		...
		##caching the current device state
		#print cacheSize
		if int(cached)==1:
			#cacheMiss=cacheMiss+1
			pass
		elif device_name[0] in cache.keys() and cacheSize!=0:
			get_red('Updating the cache in Gateway-2.')
			...
			if len(cache)==cacheSize:
				get_red('Cache full in GATEWAY-2 Evicting the cache using LRU and caching the current item')
				lru(device_name[0], state, timestamp, ID, clock)
				cacheMiss=cacheMiss+1
			else:		
				get_red('Adding the cache in Gateway-2.')
				cache[device_name[0]].append(timestamp)
				cache[device_name[0]].append(ID)
				cache[device_name[0]].append(state)
				cache[device_name[0]].append(clock)
	...
	def eventplot(sequence):
		global plot_value
		backend = xmlrpclib.ServerProxy('http://localhost:9008')
...
#################back end client#########################################
def ClientFrontend():
	gateway_proxy = xmlrpclib.ServerProxy('http://localhost:9001')
	backend_1 = xmlrpclib.ServerProxy('http://localhost:9008')
	backend_2 = xmlrpclib.ServerProxy('http://localhost:8008')
	eventnotify.set()
	while True:
		eventReport.wait()
		eventReport.clear()
...
def ClientHeartbeat():
	global nodeID_dict
	time.sleep(1.5)
	replica=xmlrpclib.ServerProxy('http://localhost:8001')
	try:
		...
		get_red('GATEWAY-1 FAILED. INITIATE FAILURE RECOVERY.')
...
def main():
	...
	#time.sleep(0.02)
	print "Total time taken to complete the GATEWAY-2 Program is", end-start
