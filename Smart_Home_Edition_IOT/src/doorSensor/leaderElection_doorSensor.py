...
def RPCClient():
	...
	'''if start_node==1:  ### explicityly set the start node
		eventRPC.set()
		start_node = 0'''
	node = xmlrpclib.ServerProxy('http://localhost:8013')
	while True:#node.get_state() == "normal":
		...
		if (node.is_alive()):
			...
			time.sleep(0.5)
			try:
				s1 = xmlrpclib.ServerProxy('http://localhost:'+str(8000+3))#int(node.get_connected_to())))
...
def RPCClient_heartbeat():
	global start_node
	app = xmlrpclib.ServerProxy('http://localhost:8003')
	node = xmlrpclib.ServerProxy('http://localhost:8013')
