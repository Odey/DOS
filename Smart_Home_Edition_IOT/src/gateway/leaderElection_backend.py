...
def RPCServer():
	class RequestHandler(SimpleXMLRPCRequestHandler):
		rpc_paths = ('/RPC2',)
	# Create server
	server = SimpleXMLRPCServer(("localhost", 8018), requestHandler=RequestHandler,logRequests=False, allow_none=True)
...
def RPCClient():
	...
	'''if start_node==1:  ### explicityly set the start node
		eventRPC.set()
		start_node = 0'''
	node = xmlrpclib.ServerProxy('http://localhost:8018')
	while True:#node.get_state() == "normal":
		...
		if (node.is_alive()):
			...
			time.sleep(0.5)
			try:
				s1 = xmlrpclib.ServerProxy('http://localhost:'+str(8000+8))#int(node.get_connected_to())))
...
def RPCClient_heartbeat():
	global start_node
	app = xmlrpclib.ServerProxy('http://localhost:8008')
	node = xmlrpclib.ServerProxy('http://localhost:8018')
	...
	while(node.is_alive() and app.is_alive()):
		try:
			...
			if int(neighbor)==int(node.get_leader()):
				print "Neighbor down ... Starting Leader Election again from backend"
				start_node = 1
				eventRPC.set()
