...
def RPCClient():
	...
	if start_node==1:  ### explicityly set the start node
		...
		start_node = 0
	node = xmlrpclib.ServerProxy('http://localhost:8011')
	while True:#node.get_state() == "normal":
		...
		if (node.is_alive()):
			...
			time.sleep(0.5)
			try:
				s1 = xmlrpclib.ServerProxy('http://localhost:8001')
...
def RPCClient_heartbeat():
	global start_node
	app = xmlrpclib.ServerProxy('http://localhost:8001')
	node = xmlrpclib.ServerProxy('http://localhost:8011')
	neighbor = node.get_neighbors()[0]
	if start_node==1:  ### explicityly set the start node
		eventRPC.set()
		start_node = 0
	while(app.is_alive()):#and node.is_alive()):
		try:
			neighbor_app = xmlrpclib.ServerProxy('http://localhost:'+str(8000+int(neighbor))) ## check with applcation
			while(neighbor_app.is_alive()):
				pass
		except:
			if int(neighbor)==int(node.get_leader()):
				print "Neighbor down ... Starting Leader Election again from frontend"
				start_node = 1
				eventRPC.set()
