...
def RPCClient():
	...
	##wait for an event from the users or Gateway.	
	...
	while True:		
		...
		event.clear()
		#proc = ['Bu','G']
		proxyGateway.report_state(bulb.get_timestamp(),bulb.get_ID(bulb.Clock()), bulb.get_state(bulb.Clock()), bulb.Clock(),['smartBulb','G'])
		event.clear()
