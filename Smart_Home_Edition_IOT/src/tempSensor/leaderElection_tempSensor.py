...
def RPCServer():
	class RequestHandler(SimpleXMLRPCRequestHandler):
		rpc_paths = ('/RPC2',)
	# Create server
	server = SimpleXMLRPCServer(("localhost", 8017), requestHandler=RequestHandler,logRequests=False, allow_none=True)
...
def RPCClient():
	...
	'''if start_node==1:  ### explicityly set the start node
		eventRPC.set()
		start_node = 0'''
	node = xmlrpclib.ServerProxy('http://localhost:8017')
	while True:#node.get_state() == "normal":
		...
		if (node.is_alive()):
			...
			time.sleep(0.5)
			try:
				s1 = xmlrpclib.ServerProxy('http://localhost:'+str(8000+7))#int(node.get_connected_to())))
...
def RPCClient_heartbeat():
	global start_node
	app = xmlrpclib.ServerProxy('http://localhost:8007')
	node = xmlrpclib.ServerProxy('http://localhost:8017')
